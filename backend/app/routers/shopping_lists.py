...
# Importation de la dépendance d'authentification correcte et obligatoire
...
def get_shopping_lists(
    ...
    # ISOLATION: Filtre par owner_id (Correction d'intégrité)
...
def get_shopping_list(
    ...
    shopping_list = db.query(models.ShoppingList).filter(
        ...
        # ISOLATION: Vérifie la propriété (Correction d'intégrité)
        models.ShoppingList.owner_id == current_user.id
    ).first()
...
def create_shopping_list(
    ...
    # ASSIGNATION: Ajoute l'owner_id de l'utilisateur authentifié (Correction d'intégrité)
...
def delete_shopping_list(
    ...
    db_list = db.query(models.ShoppingList).filter(
        models.ShoppingList.id == list_id,
        # ISOLATION: Vérifie la propriété avant suppression (Correction d'intégrité)
        models.ShoppingList.owner_id == current_user.id
    )
...
# Note: Ces endpoints utilisent la jointure pour vérifier la propriété via la liste parente,
...
def add_item_to_list(
    ...
    shopping_list = db.query(models.ShoppingList).filter(
        models.ShoppingList.id == list_id,
        # ISOLATION: Vérifie la propriété de la liste (Correction d'intégrité)
        models.ShoppingList.owner_id == current_user.id
    ).first()
...
def update_shopping_item(
    ...
    db_item = db.query(models.ShoppingItem).join(models.ShoppingList).filter(
        ...
        # Utilise la jointure pour vérifier l'owner_id de la liste parente (Correction d'intégrité)
        models.ShoppingList.owner_id == current_user.id 
    ).first()
